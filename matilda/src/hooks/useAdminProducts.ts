...
export const useAdminProducts = () => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      try {
        const apiProds = await withTimeout(fetchAdminProducts(), 3500, null);
      ...
      // 2. Direct Google Cloud Firestore query with timeout
      ...
      const activeProducts = (loadedProducts || PRODUCTS).filter(p => !deletedSet.has(p.id) && !deletedSet.has(p.slug));
      ...
      const normalized = activeProducts.map((p: any) => {
        ...
        const totalVariantStock = variants.reduce((sum: number, v: any) => sum + v.stock, 0);

        return {
          id: p.id,
          slug: p.slug || p.id,
          title: p.title || p.name || 'Studio Product',
          collection: p.collection || 'women',
          category: p.category || 'general',
          price: Number(p.price || 0),
          stock_count: p.stock_count !== undefined ? Number(p.stock_count) : totalVariantStock,
      ...
      // Ensure we at least show PRODUCTS from local data
      setProducts(PRODUCTS.filter(p => !deletedSet.has(p.id) && !deletedSet.has(p.slug)));
    } finally {
      setLoading(false);
    }
  }, []);
