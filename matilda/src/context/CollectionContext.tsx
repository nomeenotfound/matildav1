      ...
      supabase.from('products').select('*').then(({ data, error }) => {
        if (!error && data && data.length > 0) {
          setProducts(data.map((p: any) => ({
            ...
            collection: p.category === 'men' ? 'men' : 'women',
            category: 'general',
            price: Number(p.price),
            description: p.description,
            details: [],
            mainImage: p.image_url || p.mainImage,
            lifestyleImage: p.hover_image_url || p.lifestyleImage || p.image_url || p.mainImage,
            variants: Array.isArray(p.variants) ? p.variants.map((v: any, idx: number) => ({
              id: v.id || `v${idx}`,
              name: v.size || v.name,
              inStock: typeof v.stock !== 'undefined' ? v.stock > 0 : true
            })) : [],
            material: p.material || ''
          })));
        }
