...
export const CollectionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Sync load products and categories from backend API / Supabase / localStorage
  const refreshCatalogue = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(fetchedProds) && fetchedProds.length > 0) {
        const normalized = fetchedProds.map((p: any) => {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('matilda-catalogue-updated', handleCatalogueUpdate);
  }, [refreshCatalogue]);
