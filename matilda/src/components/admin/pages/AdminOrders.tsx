...
export const AdminOrders: React.FC = () => {
  ...
  const sendConfirmation = (order: any) => {
    ...
    itemsList.forEach((item: any) => {
      itemsText += `- ${item.quantity}x ${item.product.title} (${item.selectedVariant?.name || 'Standard'})\n`;
    });

    const text = `Hi ${order.customer_name},\n\nThank you so much for shopping with Matilda Studio! ❤️\n\nYour payment has been successfully verified. Here are your order details:\n\n*Order Number:* ${order.order_number}\n*Total Amount:* ₹${order.total_amount}\n\n*Items:*\n${itemsText}\nWe will notify you once your order is shipped.\n\nWarm regards,\nDuha\nMatilda Studio`;

    const encoded = encodeURIComponent(text);
    const phone = order.phone.replace(/[^0-9]/g, '');
    window.open(`https://wa.me/91${phone.length === 10 ? phone : phone.replace(/^91/, '')}?text=${encoded}`, '_blank');
  };
          ...
          {filteredOrders.map(order => (
            <div key={order.id} className="bg-white/70 backdrop-blur-md border border-[var(--border-admin)] rounded-3xl p-4 sm:p-6 flex flex-col gap-4 shadow-sm hover:shadow-md transition-shadow relative">
              ...
              <div className="mt-auto pt-4 border-t border-[var(--border-admin-subtle)] grid grid-cols-2 gap-2">
                ...
                {order.status === 'paid' && (
                  <>
                    <button onClick={() => sendConfirmation(order)} className="col-span-2 border border-green-600 text-green-600 bg-green-50 font-micro uppercase tracking-widest text-[10px] py-2 rounded-full hover:bg-green-600 hover:text-white transition-colors">send confirmation</button>
                    <button onClick={() => {
