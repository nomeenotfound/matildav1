...
import { Pencil, Trash2, Plus, Loader2, Image as ImageIcon, Check, Filter, Layers } from 'lucide-react';
  ...
  const handleOpenModal = (product: any = null) => {
    if (product) {
      ...
      setFormData({
        ...
        details: [''],
        galleryImages: [''],
        variants: [{ id: 'v1', name: 'One Size', inStock: true, stock: 10 }]
      });
    }
    setIsModalOpen(true);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="font-micro uppercase tracking-widest text-[10px] text-gray-500 mt-0.5">
            manage studio inventory, pricing, multiple variants, and product photography
          </p>
        </div>
      ...
      {loading && products.length === 0 ? (
        ...
        <div className="bg-white/60 border border-dashed border-gray-300 rounded-3xl p-12 text-center flex flex-col items-center justify-center gap-3">
          <p className="font-micro uppercase tracking-widest text-xs text-gray-400">
            No products found matching your search or filters.
          </p>
          <button
            onClick={() => handleOpenModal()}
            className="px-4 py-2 bg-[var(--border-admin)] text-white text-xs font-mono uppercase tracking-widest rounded-xl hover:opacity-90 transition-opacity flex items-center gap-2 mt-2"
          >
