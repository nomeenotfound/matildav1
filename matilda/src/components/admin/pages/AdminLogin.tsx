...
export const AdminLogin: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (trimmed.toUpperCase() === 'MANGO11' || trimmed.toLowerCase() === 'datmat1') {
      ...
      setError(errData.error || 'Invalid access code');
    } catch (e) {
      if (trimmed.toUpperCase() === 'MANGO11' || trimmed.toLowerCase() === 'datmat1') {
        const fallbackToken = 'matilda_mango11_session_token';
        localStorage.setItem('admin_token', fallbackToken);
      ...
      setError('Network issue logging in. Please try again.');
    } finally {
      setLoading(false);
    }
  };
