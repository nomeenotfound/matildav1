        ...
        >
          ...
          <div className="flex flex-col justify-between max-h-[calc(90vh-16rem)] md:max-h-[90vh] overflow-hidden bg-[var(--bg-primary)]">
            ...
            <div className="p-5 sm:p-6 overflow-y-auto space-y-4 flex-1">
              ...
              <div className="pr-10 sm:pr-12 space-y-1.5">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[var(--border-maroon)] font-matilda text-xs font-bold uppercase tracking-wider">
                    {selectedProduct.category}
                  </span>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold lowercase ${
              ...
              {selectedProduct.variants && selectedProduct.variants.length > 0 && (
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center text-[11px]">
                    ...
                    <span className="text-[var(--text-muted)] lowercase font-medium">
                      {selectedVariant?.name}
                      {selectedVariant && (
                        <span className="ml-1 text-[10px] text-gray-500 font-normal">
                          ({selectedVariant.stock !== undefined ? selectedVariant.stock : (selectedVariant.inStock ? 10 : 0)} in stock)
                        </span>
                      )}
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-1.5 text-xs">
                    {selectedProduct.variants.map((v) => {
                      const vOutOfStock = v.stock !== undefined ? v.stock <= 0 : !v.inStock;
                      const isSelected = selectedVariant?.id === v.id;
                      return (
                        <button
                          key={v.id}
                          onClick={() => setSelectedVariant(v)}
                          className={`px-3 py-1 rounded-full text-xs lowercase transition-all duration-200 font-medium flex items-center gap-1 ${
                            isSelected
                              ? 'bg-[var(--border-maroon)] text-white font-semibold shadow-xs'
                              : vOutOfStock
                                ? 'bg-gray-100/60 text-gray-400 border border-gray-200 opacity-60'
                ...
                return (
                  <div className="grid grid-cols-2 gap-2.5 pt-1">
                    <button
                      ...
                      className={`w-full py-3 px-4 rounded-full text-xs lowercase font-bold transition-all flex items-center justify-center gap-2 shadow-xs ${
                        isSelectedOutOfStock
                          ? 'bg-gray-200 text-gray-400 cursor-not-allowed border border-gray-300'
                          : 'bg-[var(--card-bg)] hover:bg-[var(--card-inner)] text-[var(--text-dominant)] border border-[var(--border-main)]/50 active:scale-[0.98]'
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`w-full py-3 px-4 rounded-full text-xs lowercase font-bold transition-all flex items-center justify-center gap-2 shadow-md ${
                        isSelectedOutOfStock
                          ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                          : 'bg-[var(--border-maroon)] text-white hover:bg-[var(--text-dominant)] active:scale-[0.98]'
                      }`}
                    >
