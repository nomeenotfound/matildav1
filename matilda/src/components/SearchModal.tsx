...
export const SearchModal: React.FC = () => {
  ...
  const popularTerms = ['silver', 'wool', 'ceramics', 'leather', 'rings', 'shirts'];

  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-start justify-center pt-12 sm:pt-20 px-4 bg-black/60 backdrop-blur-sm">
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          ...
          <div className="relative mt-4 mb-4">
            ...
            <input
              ...
              placeholder="type silver, wool, ceramics or leather..."
          ...
          {query.trim() === '' && (
            <div className="py-3">
              <span className="text-[11px] font-bold lowercase text-[var(--text-muted)] block mb-2">
                popular searches
              </span>
              <div className="flex flex-wrap gap-2 text-xs">
                {popularTerms.map((term) => (
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg border border-[var(--border-main)] bg-[var(--card-bg)] hover:border-[var(--border-maroon)] text-[var(--text-primary)] transition-all font-medium text-xs lowercase"
                  >
                    {term}
                  </button>
                ))}
              </div>
              <p className="mt-5 text-xs text-[var(--text-muted)] leading-relaxed lowercase">
                type any word to search through our quiet valley collection.
              </p>
            </div>
          )}
