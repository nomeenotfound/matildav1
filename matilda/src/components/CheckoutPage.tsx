...
import { ArrowLeft, Copy, Check, UploadCloud, Truck, MapPin, Loader2 } from 'lucide-react';
...
export const CheckoutPage: React.FC = () => {
  ...
  const isOverLimit = finalTotal > MAX_ORDER_AMOUNT;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isOverLimit) {
      setError(`maximum order amount is ₹${MAX_ORDER_AMOUNT.toLocaleString('en-IN')} at once. please reduce items in your bag or place separate orders.`);
      return;
    }
          ...
          <section className="bg-[var(--bg-primary)]/40 backdrop-blur-xl border border-[var(--border-main)]/10 rounded-3xl p-6 sm:p-8 shadow-sm">
            ...
            <div className="flex flex-col xl:flex-row items-center xl:items-start gap-8">
              ...
              <div className="flex flex-col items-center xl:items-start w-full">
                <div className="mb-4 w-full space-y-1.5 bg-[var(--bg-primary)]/70 p-4 rounded-2xl border border-[var(--border-main)]/15 text-xs">
                  <div className="flex justify-between items-center text-[var(--text-muted)] lowercase">
                    <span>items subtotal</span>
                    <span className="font-semibold text-[var(--text-dominant)]">₹{itemsSubtotal.toLocaleString('en-IN')}</span>
                  </div>
                  <div className="flex justify-between items-center text-[var(--text-muted)] lowercase">
                    <span>shipping</span>
                    {isPincodeValid && shippingCalc ? (
                      <span className="font-semibold text-[var(--text-dominant)]">₹{deliveryFee}</span>
                    ) : (
                      <span className="font-medium italic text-[var(--text-muted)]">please enter pincode to calculate shipping</span>
                    )}
                  </div>
                  {discountAmount > 0 && (
                    <div className="flex justify-between items-center text-green-700 lowercase font-medium">
                      <span>{activeDiscountLabel}</span>
                      <span>-₹{discountAmount.toLocaleString('en-IN')}</span>
                    </div>
                  )}
                  <div className="flex justify-between items-center pt-2 border-t border-[var(--border-main)]/15 text-sm font-bold lowercase text-[var(--text-dominant)]">
                    <span>total payable</span>
                    <span className="text-2xl font-display font-extrabold text-[var(--border-maroon)]">
                      ₹{finalTotal.toLocaleString('en-IN')}
                      {!isPincodeValid && <span className="text-xs font-normal text-[var(--text-muted)] ml-1.5">(+ shipping)</span>}
                    </span>
                  </div>
                </div>
                ...
                <a 
                  ...
                  className="mt-4 md:hidden w-full bg-[var(--border-maroon)] text-white py-3.5 rounded-2xl flex items-center justify-center font-display font-bold lowercase tracking-wide shadow-md hover:opacity-90 transition-opacity"
                >
          ...
          <div className="pt-2">
            {isOverLimit && (
              <div className="text-amber-800 bg-amber-50 font-display font-medium text-xs lowercase p-4 rounded-2xl border border-amber-200 mb-4 text-center">
                the maximum order amount is ₹{MAX_ORDER_AMOUNT.toLocaleString('en-IN')} at once. please reduce items in your bag or place separate orders.
              </div>
            )}
            ...
            <button form="checkout-form" type="submit" disabled={loading || isOverLimit} className="w-full py-4 bg-[var(--border-maroon)] text-white font-medium lowercase tracking-wide text-sm rounded-full hover:bg-[var(--text-dominant)] transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm">
              {loading ? 'processing...' : isOverLimit ? `exceeds ₹${MAX_ORDER_AMOUNT} limit` : 'complete order'}
            </button>
          </div>
        </div>
      </div>
