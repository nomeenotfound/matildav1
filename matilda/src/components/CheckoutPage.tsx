    ...
    try {
      ...
      try {
        ...
        if (res.ok && json?.orderNumber) {
          finalOrderNumber = json.orderNumber;
        } else if (json?.error) {
          throw new Error(json.error);
        } else if (!res.ok) {
          // If server returned error response, attempt direct client-side save via Supabase if configured
          if (supabase) {
            ...
            throw new Error("Unable to complete order at this time. Please try again or contact support.");
          }
        }
      } catch (err: any) {
        const msg = err?.message || '';
        if (msg && !msg.toLowerCase().includes("json") && !msg.toLowerCase().includes("unexpected token") && !msg.toLowerCase().includes("not valid")) {
          throw err;
        }
        // If JSON parse error, direct write to Supabase if available
        if (supabase) {
          const finalUtr = isCOD ? 'COD - Cash on Delivery' : formData.utr;
          await supabase.from('orders').insert({
            order_number: finalOrderNumber,
          ...
          throw new Error("Server error. Please check your connectivity and try again.");
        }
      }
      ...
      let friendlyError = err?.message || 'An error occurred while placing your order. Please try again.';
      if (friendlyError.toLowerCase().includes('json') || friendlyError.toLowerCase().includes('unexpected token') || friendlyError.toLowerCase().includes('is not valid')) {
        friendlyError = 'Network or server error while placing order. Please try again.';
      }
