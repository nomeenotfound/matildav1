...
export const CheckoutPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Fetch store settings directly from Supabase with graceful fallback
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Submit order to Express Backend API as primary channel
      try {
        ...
        if (apiRes.ok) {
          ...
          if (resData && resData.orderNumber) {
            finalOrderNumber = resData.orderNumber;
          }
        }
      } catch (apiErr) {
        console.warn('Backend API checkout submission notice (proceeding with local/db backups):', apiErr);
      }
      ...
      // 2. Store order copy in browser localStorage for offline & instant tracking access
      try {
        const existingLocal = localStorage.getItem('matilda_local_orders');
        const ordersArr = existingLocal ? JSON.parse(existingLocal) : [];
        ordersArr.unshift(orderRecord);
        localStorage.setItem('matilda_local_orders', JSON.stringify(ordersArr));
      } catch (lsErr) {
        console.warn('LocalStorage save notice:', lsErr);
      }

      // 2. Direct Supabase database insertion (if Supabase is configured)
      if (supabase) {
        try {
          // Upload screenshot to Supabase Storage if file exists
          let screenshotUrl = screenshotBase64;
          if (!isCOD && file) {
            try {
              const fileExt = file.name.split('.').pop() || 'png';
              const fileName = `proof_${finalOrderNumber}_${Date.now()}.${fileExt}`;
          ...
          const sbInsertRecord = {
            ...orderRecord,
            payment_screenshot: screenshotUrl || null
          };

          const { error: sbErr } = await supabase.from('orders').insert(sbInsertRecord);
          if (sbErr) {
            console.warn('Supabase insert notice (order saved locally):', sbErr.message);
            // Try inserting without large image if payload was too large
            if (sbErr.message?.toLowerCase().includes('large') || sbErr.message?.toLowerCase().includes('payload')) {
          ...
          if (formData.phone && formData.phone.trim()) {
            try {
              ...
              console.warn('Customer update notice:', cErr);
            }
          }
        } catch (dbErr) {
          console.warn('Supabase operation notice (proceeding with local order):', dbErr);
        }
      }
