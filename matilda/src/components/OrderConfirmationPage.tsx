...
export const OrderConfirmationPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchOrderDetails = async () => {
      try {
        const res = await fetch(`/api/orders/details?order=${encodeURIComponent(rawOrderNum)}`);
        if (!res.ok) {
          // If not found in database, check if we have state from checkout page
          ...
          throw new Error('Order reference could not be found.');
        }
        ...
        if (isMounted) {
          if (location.state && (location.state as any).orderNumber) {
            const st = location.state as any;
            setOrder({
              order_number: st.orderNumber,
              customer_name: st.customerName || 'Customer',
