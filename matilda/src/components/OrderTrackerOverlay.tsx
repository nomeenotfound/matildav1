      ...
      try {
        ...
        if (supabase) {
          const { data: sbOrder } = await supabase
            .from('orders')
            .select('status, tracking_number, courier_name, utr_number, rejection_reason')
            .eq('order_number', orderNumber)
            .maybeSingle();

          if (sbOrder) {
            const isCod = sbOrder.utr_number?.toUpperCase().includes('COD');
            setStatusData({
              status: sbOrder.status || 'pending',
              tracking_info: sbOrder.tracking_number ? `${sbOrder.courier_name || 'Delhivery'}: ${sbOrder.tracking_number}` : undefined,
              rejection_reason: sbOrder.rejection_reason,
              is_cod: isCod
            });
            setLoading(false);
            return;
        ...
        // Default fallback if not found in Supabase or localStorage
        if (!statusData) {
          setStatusData({
            status: 'pending',
            is_cod: false
