...
async function initServerFirestore() {
  ...
  try {
    ...
    return await withTimeoutServer(initPromise, 3000, null);
  } catch (err) {
...
(async () => {
  try {
    ...
    if (db && _fbCollection && _fbGetDocs) {
      ...
      // 2. Sync categories
      const catSnap = await _fbGetDocs(_fbCollection(db, 'categories'));
      if (!catSnap.empty) {
        const fsCats: any[] = [];
      ...
      const prodSnap = await _fbGetDocs(_fbCollection(db, 'products'));
      if (!prodSnap.empty) {
        const fsProds: any[] = [];
        ...
        console.log(`[Firestore] Seeding ${inMemoryProducts.length} catalog products to Firestore...`);
...
app.get(["/api/config/public", "/api/config"], (req, res) => {
  ...
  res.json({
    firebase: {
      projectId: process.env.FIREBASE_PROJECT_ID || process.env.VITE_FIREBASE_PROJECT_ID || process.env.GOOGLE_CLOUD_PROJECT || "",
      apiKey: process.env.FIREBASE_API_KEY || process.env.VITE_FIREBASE_API_KEY || "",
      authDomain: process.env.FIREBASE_AUTH_DOMAIN || process.env.VITE_FIREBASE_AUTH_DOMAIN || "",
      storageBucket: process.env.FIREBASE_STORAGE_BUCKET || process.env.VITE_FIREBASE_STORAGE_BUCKET || "",
...
app.get(["/api/store/settings", "/store/settings"], async (req, res) => {
  ...
  const currentDiskSettings = loadPersistedSettings();
  let mergedSettings = { ...defaultSettings, ...currentDiskSettings, ...inMemorySettings };

  try {
    const db = await initServerFirestore();
    if (db && _fbCollection && _fbGetDocs) {
      const snap = await _fbGetDocs(_fbCollection(db, 'store_settings'));
      if (!snap.empty) {
        snap.forEach((d: any) => {
          const item = d.data();
          if (item) {
...
app.get(["/api/products", "/products"], async (req, res) => {
  try {
    ...
    try {
      const db = await initServerFirestore();
      if (db && _fbCollection && _fbGetDocs) {
        const snap = await _fbGetDocs(_fbCollection(db, 'products'));
        if (!snap.empty) {
          snap.forEach((d: any) => {
            const data = d.data();
    ...
    if (productsList.length === 0) {
      productsList = inMemoryProducts.length > 0 ? [...inMemoryProducts] : loadDefaultProducts();
    }
    ...
    res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate');
    res.json(inMemoryProducts.filter(p => !deletedProductIds.has(p.id) && !deletedProductIds.has(p.slug)));
  }
});
...
function loadDefaultProducts(): any[] {
  for (const fp of PRODUCTS_STORAGE_PATHS) {
    try {
      ...
      console.warn("Failed loading products from disk, trying next path:", fp, e);
    }
  }
  return [
    {
      id: 'golden-blob-studs-',
      slug: 'golden-blob-studs-',
      title: 'golden blob studs',
      collection: 'women',
      category: 'studs',
      price: 89,
      stock_count: 1,
      description: "simple, stunning, and made for every day.",
      details: ["shiny golden look", "royal feminine-esque"],
      mainImage: 'https://wagfmihahpdgoqgghpzf.supabase.co/storage/v1/object/public/product-images/1786737257179-ed3nw.jpeg',
      lifestyleImage: 'https://wagfmihahpdgoqgghpzf.supabase.co/storage/v1/object/public/product-images/1786737268186-r3d1wi.jpeg',
      galleryImages: [],
      imageFit: 'cover',
      variants: [{ id: 'v1', name: 'One Size', stock: 1, inStock: true }],
      isFeatured: false,
      hasVictorianFrame: false,
      material: 'golden bakelite'
    }
  ];
}
