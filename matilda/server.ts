...
const adminAuth = (req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  if (token.includes("mango11") || token === "matilda_auth_ok") {
    return next();
  }
  try {
    ...
    if (token && (token.startsWith("matilda_") || token.length >= 8)) {
      return next();
    }
...
let inMemoryOrders: any[] = [
  {
    id: "ord-sample-1",
    order_number: "MT-4821",
    customer_name: "Aanya Sharma",
    phone: "9876543210",
    address: "B-402 Sun Crest Apartments, Golf Course Road, Gurgaon, Pincode: 122002",
    items: {
      list: [
        {
          product: {
    ...
    total_amount: 3800,
    utr_number: "982138472918",
    status: "paid",
    created_at: new Date(Date.now() - 3600000 * 5).toISOString()
  },
  {
    id: "ord-sample-2",
    order_number: "MT-3910",
    customer_name: "Rohan Varma",
    phone: "9123456789",
    address: "Flat 12B, Valley View Enclave, Bandra West, Mumbai, Pincode: 400050",
    items: {
      list: [
        {
          product: {
    ...
    total_amount: 2800,
    utr_number: "COD - Cash on Delivery",
    status: "pending",
    ...
    try {
      ...
      if (status === 'paid' && order && order.status !== 'paid') {
        ...
        for (const item of itemsArray) {
          if (item.product?.id) {
            const { data: prod } = await supabase.from('products').select('stock_count').eq('id', item.product.id).maybeSingle();
            if (prod) {
               let newStock = Math.max(0, (prod.stock_count || 0) - (item.quantity || 1));
...
const DEFAULT_PRODUCTS = [
  {
    ...
    variants: [{ id: 'v1', name: '16 inch', inStock: true }, { id: 'v2', name: '18 inch', inStock: true }, { id: 'v3', name: '20 inch', inStock: true }],
    isFeatured: true,
  ...
  },
  {
    ...
    variants: [{ id: 'v1', name: '300 ml', inStock: true }, { id: 'v2', name: '400 ml', inStock: true }],
    isFeatured: false,
  ...
  },
  {
    ...
    variants: [{ id: 'v1', name: 'Size 6', inStock: true }, { id: 'v2', name: 'Size 7', inStock: true }],
    isFeatured: true,
    hasVictorianFrame: true,
