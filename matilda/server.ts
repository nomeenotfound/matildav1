  ...
  try {
    ...
    if (!isCOD) {
      if (file) {
        ...
        if (screenshotInput.startsWith('data:image/') && supabase) {
          try {
            const match = screenshotInput.match(/^data:(image\/\w+);base64,(.+)$/);
            if (match) {
    ...
    return res.status(400).json({ error: e?.message || "Failed to place order. Please check your details and try again." });
  }
});
