...
app.get(["/api/products", "/products"], async (req, res) => {
  ...
  try {
    ...
    if (db && _fbCollection && _fbGetDocs) {
      ...
      if (!snap.empty) {
        snap.forEach((d: any) => productsList.push(d.data()));
        if (productsList.length > 0) {
          inMemoryProducts = productsList;
...
app.post("/api/admin/products", adminAuth, async (req, res) => {
  ...
  inMemoryProducts = inMemoryProducts.filter(p => p.id !== newProd.id);
...
app.delete("/api/admin/products/:id", adminAuth, async (req, res) => {
  ...
  try {
    const db = await initServerFirestore();
    if (db && _fbDoc && _fbDeleteDoc) {
      await _fbDeleteDoc(_fbDoc(db, 'products', id));
    }
  } catch (e) {
