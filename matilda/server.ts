...
function loadDefaultProducts(): any[] {
  try {
    const pJsonPath = path.join(process.cwd(), 'src', 'data', 'products.json');
    if (fs.existsSync(pJsonPath)) {
      const data = JSON.parse(fs.readFileSync(pJsonPath, 'utf-8'));
      if (Array.isArray(data) && data.length > 0) {
        return data;
      }
    }
  } catch (e) {
    console.warn("Failed loading products.json, using fallback:", e);
  }
  return [
    {
...
app.post("/api/admin/products", adminAuth, async (req, res) => {
  ...
  try {
    const db = await initServerFirestore();
    if (db && _fbDoc && _fbSetDoc) {
      await _fbSetDoc(_fbDoc(db, 'products', newProd.id), newProd);
    }
  } catch (e) {
...
app.put("/api/admin/products/:id", adminAuth, async (req, res) => {
  ...
  try {
    const db = await initServerFirestore();
    if (db && _fbDoc && _fbUpdateDoc) {
      await _fbUpdateDoc(_fbDoc(db, 'products', id), updatedProd);
    }
  } catch (e) {
