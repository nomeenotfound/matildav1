  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2 font-micro text-[10px] uppercase tracking-widest">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredOrders.map(order => (
            <div key={order.id} className="bg-white/70 backdrop-blur-md border border-[var(--border-admin)] rounded-3xl p-6 flex flex-col gap-4 shadow-sm hover:shadow-md transition-shadow relative">
              <button 
                onClick={() => setSelectedOrder(order)}
                className="absolute top-6 right-6 text-gray-400 hover:text-[var(--border-admin)]"
                title="View Details"
              >
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl p-8 max-w-2xl w-full border border-[var(--border-admin)] shadow-xl relative max-h-[90vh] overflow-y-auto">
            <button onClick={() => setSelectedOrder(null)} className="absolute top-6 right-6 text-gray-400 hover:text-black">
              <X className="w-5 h-5" />
            </button>
